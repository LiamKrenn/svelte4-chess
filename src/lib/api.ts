import type { Chessground } from "svelte4-chessground";
...
export class Api {
  ...
  constructor(
    ...
    private promotionCallback: (sq: Square) => Promise<PieceSymbol> = async (
      sq
    ) => "q", // called before promotion
  ...
  async _chessgroundMoveCallback(orig: Square | "a0", dest: Square | "a0") {
    ...
    if (this.engine && this.engine.isSearching()) this.engine.stopSearch();
    let cjsMove: CjsMove;
  ...
  move(
    moveSanOrObj: string | { from: string; to: string; promotion?: string }
  ) {
    if (!this.initialised)
